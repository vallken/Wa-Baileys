...
async function downloads2(url) {
  ...
  if (url.includes("spotify.link")) {
    ...
    const track = await axios
      .get(`https://spotifydownloaders.com/api/spotify?url=${url}`, options2)
      .then((res) => res.buffer());
    return track.data;
...
async function downloads(url) {
  ...
  if (url.includes("spotify.link")) {
    ...
    const audioUrl = await axios
      .get(data.link, { responseType: "arrayBuffer" })
      .then((res) => Buffer.from(res.data));
    const imgUrl = await axios
      .get(data.metadata.cover, {responseType: "arrayBuffer" })
      .then((res) => Buffer.from(res.data));
    ...
    const audioUrl = await axios.get(data.link, {responseType: "arrayBuffer"}).then((res) => Buffer.from(res.data));
    const imgUrl = await axios.get(data.metadata.cover, {responseType: "arrayBuffer"}).then((res) => Buffer.from(res.data));
    const writeTags = tags2(
      data.metadata.title,
      data.metadata.artists,
      data.metadata.releaseDate,
...
async function downloadAlbum2(url) {
  ...
  try {
    ...
    const trackDetails = await axios
      .get(
        `https://spotifydownloaders.com/api/getSpotifyDetails?url=${urll}`,
        options2
      )
      .then((res) => res.data);
    ...
    for (let i = 0; i < trackDetails.tracks.length; i++) {
      const audioMp3 = await downloads2(
        `https://open.spotify.com/track/${
          trackDetails.tracks[i].uri.split("track:")[1]
        }`
      );
...
async function downloadTrack2(song) {
  ...
  if (isUrl(song)) {
    try {
      if (song.includes("spotify.link")) {
        ...
        var tracks = await spotify.getTrack(
          dataSong.split("track/")[1].split("?")[0]
        );
    ...
    try {
      ...
      const downloadData = await downloads2(
